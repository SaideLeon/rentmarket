  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err?.code === 'auth/configuration-not-found' || err?.message?.includes('configuration-not-found')) {
        setAuthConfigError('O fornecedor Google de Autenticação não está ativo na Consola Firebase. Por favor ative a autenticação Google no Firebase Console.');
        showToast('Ative a Autenticação Google no Firebase Console.', 'info');
      } else {
